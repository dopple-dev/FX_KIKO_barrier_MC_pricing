...
class barrier_price:
    ...
    def root_chi2(a, b, u):
        ...
        c0 = a
        bnds = [(0., None)]
        res = minimize(barrier_price.equation, c0, args=(a, b, u), bounds=bnds)
        return res.x[0]
    
    def sabrMC(F0=0.04, sigma0=0.07, nu=0.5, beta=0.25, rho=0.4, psi_threshold=2., n_years=1.0, T=252, N=1000):
        """
        ...
        W2t = barrier_price.simulate_Wt(dW2, T, N)
        
        # volatility process
        sigma_t = barrier_price.simulate_sigma(W2t, sigma0, nu, t)
        
        # integrated variance-values = integrals between ti-1 and ti (not over entire interval [0,ti])
        v_t = barrier_price.integrated_variance_small_disturbances(N, rho, nu, sigma_t, dt, dW2, U1)
        ...
        for n in range(0, N):
            for ti in range(1, T):
                ...
                pr_zero = barrier_price.AbsorptionConditionalProb(a, b)
                
                if pr_zero > U[ti - 1, n]:
                    Ft[ti, n] = 0.
                    continue
                
                m, psi = barrier_price.andersen_QE(a, b)
    ...
    def option_price(barrier_type, MC_T, K, mask):
    ...
    def MC_barrier_pricing(barrier_type, H, F0, rd, K, nu, alpha, beta, rho, psi_threshold, tau, n_MC, n_steps, n_simulation):
        ...
        for i in range(n_MC):
            ...
            MC_T = barrier_price.sabrMC(F0, nu, alpha, beta, rho, psi_threshold, tau, n_steps, n_simulation).T
            ...
            barrier[i] = barrier_price.option_price(barrier_type, MC_T, K, mask)
